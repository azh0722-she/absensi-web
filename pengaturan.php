...
// Ambil profil admin + username dari tabel users
...
// Jika belum ada profil admin, buat entry kosong
if(!$admin){
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if(isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK){
        ...
        if(move_uploaded_file($fileTmp, $targetFile)){
            $stmt3 = mysqli_prepare($conn, "UPDATE admin SET foto=? WHERE user_id=?");
    ...
    $stmt = mysqli_prepare($conn, "UPDATE admin SET nama_lengkap=?, email=?, alamat=?, no_telp=?, tempat_lahir=?, tanggal_lahir=?, jenis_kelamin=? WHERE user_id=?");
    mysqli_stmt_bind_param($stmt, "sssssssi", $nama, $email, $alamat, $no_telp, $tempat_lahir, $tanggal_lahir, $jenis_kelamin, $user_id);
    mysqli_stmt_execute($stmt);
    ...
    if(!empty($username_new) && $username_new !== $admin['username']){
    ...
    <div class="content col">
        <div class="profile-card">
            ...
            <form method="post" enctype="multipart/form-data">
                <img id="preview-foto" src="<?= !empty($admin['foto']) ? $admin['foto'] : 'https://cdn-icons-png.flaticon.com/512/847/847969.png' ?>" onclick="document.getElementById('foto').click();" alt="Foto Profil">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="nama" name="nama" value="<?= htmlspecialchars($admin['nama_lengkap']) ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="alamat" name="alamat" value="<?= htmlspecialchars($admin['alamat']) ?>">
                </div>

                <div class="form-group">
                    <label for="no_telp">No. Telepon</label>
                    <input type="text" id="no_telp" name="no_telp" value="<?= htmlspecialchars($admin['no_telp']) ?>">
                </div>

                <div class="form-group">
                    <label for="tempat_lahir">Tempat Lahir</label>
                    <input type="text" id="tempat_lahir" name="tempat_lahir" value="<?= htmlspecialchars($admin['tempat_lahir']) ?>">
                </div>

                <div class="form-group">
                    <label for="tanggal_lahir">Tanggal Lahir</label>
                    <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?= htmlspecialchars($admin['tanggal_lahir']) ?>">
                </div>

                <div class="form-group">
                    <label for="jenis_kelamin">Jenis Kelamin</label>
                    <select id="jenis_kelamin" name="jenis_kelamin">
                        <option value="" <?= $admin['jenis_kelamin']==''?'selected':'' ?>>-- Pilih --</option>
                ...
                <button type="button" class="btn-cancel" onclick="window.location.href='profil_admin.php'">↩️ Kembali ke Profil</button>
            </form>
        </div>
    </div>
</div>
