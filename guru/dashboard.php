...
// Buat entri guru kosong jika belum ada
if (!$guru) {
    $queryInsert = "INSERT INTO guru 
        (user_id, nama_lengkap, nip, email, no_telp, alamat, tempat_lahir, tanggal_lahir, jenis_kelamin, mata_pelajaran, status_guru, tanggal_masuk, foto)
        VALUES
        ($user_id, '', '', '', '', '', '', NULL, NULL, '', 'Honorer', NULL, '')";
...
<div class="d-flex">
  ...
  <div class="content col">
    ...
    <!-- Grafik Bar Chart per Status -->
    <div class="row mt-4 g-4">
      <div class="col-md-4">
        <div class="card p-3">
          <h6 class="mb-3 text-center"><i class="bi bi-check-circle"></i> Hadir</h6>
          <canvas id="chartHadir" height="120"></canvas>
        </div>
      </div>
      <div class="col-md-4">
        <div class="card p-3">
          <h6 class="mb-3 text-center"><i class="bi bi-exclamation-circle"></i> Izin</h6>
          <canvas id="chartIzin" height="120"></canvas>
        </div>
      </div>
      <div class="col-md-4">
        <div class="card p-3">
          <h6 class="mb-3 text-center"><i class="bi bi-x-circle"></i> Alfa</h6>
          <canvas id="chartAlfa" height="120"></canvas>
        </div>
      </div>
    </div>
...
fetch("sidebar.php")
  ...
  .then(html => {
      ...
      // Jalankan semua <script> yang ada di sidebar.php
  ...
  // Tambahkan event listener global setelah sidebar selesai dimuat
...
new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [
            { label:'Hadir', data: <?= json_encode($grafik_hadir) ?>, borderColor:'#22c55e', backgroundColor:'rgba(34,197,94,0.2)', fill:true, tension:0.4 },
            { label:'Izin', data: <?= json_encode($grafik_izin) ?>, borderColor:'#facc15', backgroundColor:'rgba(250,204,21,0.2)', fill:true, tension:0.4 },
            { label:'Alfa', data: <?= json_encode($grafik_alfa) ?>, borderColor:'#f87171', backgroundColor:'rgba(248,113,113,0.2)', fill:true, tension:0.4 }
        ]
    },
    options:{
        responsive:true,
...
// Grafik Bar Chart per Status
const chartsData = [
    { id:'chartHadir', label:'Hadir', data: <?= json_encode($grafik_hadir) ?>, color:'#22c55e' },
    { id:'chartIzin', label:'Izin', data: <?= json_encode($grafik_izin) ?>, color:'#facc15' },
    { id:'chartAlfa', label:'Alfa', data: <?= json_encode($grafik_alfa) ?>, color:'#f87171' }
];
...
chartsData.forEach(c=>{
    ...
    new Chart(ctx,{
        type:'bar',
        data:{ labels: <?= json_encode($grafik_labels) ?>, datasets:[{ label:c.label, data:c.data, backgroundColor:c.color }] },
        options:{
            responsive:true,
